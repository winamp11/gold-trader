...
describe('entry window', () => {
  ...
  test('21:00-23:00 is closed again — the 12 Sep extension was reverted', () => {
    // Live for ten days; mechanical went 0 for 6 in those hours for -5,762 and
    // nothing traded there profitably. Pinned so the revert is not undone by
    // accident.
    assert.equal(isEntryWindow(uae(21)), false);
    assert.equal(isEntryWindow(uae(22)), false);
    assert.equal(isTradingHours(uae(21)), false);
    assert.equal(isTradingHours(uae(22)), false);
  });

  test('the session label never outlives the window', () => {
    // getSession used to hardcode its last boundary, so moving SESSION_END
    // left 'US' labelling hours that were no longer tradeable. It now derives.
    assert.equal(getSession(uae(20, 59)), 'US');
    assert.equal(getSession(uae(21)), null);
    assert.equal(getSession(uae(22)), null);
  });
