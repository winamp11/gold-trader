...
class DatabaseService {
  ...
  async saveJournalEntry({ portfolioId, signalOrTradeId = null, entryType, lessonText, tag = null }) {
    const r = await this.pool.query(`
      INSERT INTO journal
        (portfolio_id, timestamp, signal_or_trade_id, entry_type, lesson_text, tag)
      VALUES ($1,$2,$3,$4,$5,$6)
      RETURNING id
    `, [portfolioId, new Date().toISOString(), signalOrTradeId, entryType, lessonText, tag]);
    const id = r.rows[0].id;
  ...
  async getEquityTrades(portfolioId) {
    const r = await this.pool.query(`
      SELECT exit_timestamp AS t, pnl
      FROM trades
      WHERE portfolio_id = $1 AND exit_reason IS NOT NULL AND pnl IS NOT NULL
